...
workflow assembly_based {
    ...
    take:
        file_ch // parsed csv file
        filtered_ch // ["sample_id", "assembly", "reads"]
        ko_db_dir
        cat_db
        gtdbtk_db_dir
    ...
    main:
        ...
        *****************************************************/
        ...
        assembly = filtered_ch.map{ sample_id, assembly, reads -> tuple(sample_id, file("${assembly}", checkIfExists: true))
                               }
        RENAME_HEADERS(assembly)
        ...
        reads_ch = filtered_ch.map{ sample_id, assembly, reads -> tuple(sample_id, file("${reads}", checkIfExists: true))
                               }
        MAPPING(assembly_ch.join(reads_ch))
