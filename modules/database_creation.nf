...
/*
params.krakendb_name  = null //"host"
params.krakendb_url   = null //"http://www."
params.krakendb_fasta = null // "/path/to/host_sequences.fasta"
params.krakendb_dir    = null // "path/to/host/database"
*/
...
process SETUP_KAIJU {
    ...
    tag "Downloading and setting up kaiju database..."
    label "db_setup"
    label "kaiju"

    input:
        // https://bioinformatics-centre.github.io/kaiju/downloads.html
        // nr, nr_euk, refseq, refseq_nr, refseq_ref
        // progenomes, fungi, viruses, plasmids, rvdb
        val(dbname) // nr_euk
...
https://genome-idx.s3.amazonaws.com/kraken/k2_pluspfp_20250714.tar.gz
*/
process SETUP_KRAKEN {
    ...
    script:
        ...
        mkdir kraken2-db/ && tar -zxvf kraken.tar.gz -C kraken2-db/ && \\

        # Cleaning up
        [ -f  kraken.tar.gz ] && rm -rf  kraken.tar.gz
